/**
 * Eliminator-format analysis for the Chrome extension's Eliminator Mode (ADR-011, TASK-270).
 *
 * Self-contained vanilla-ESM port of the web app's `eliminatorModel.js` (ADR-010, TASK-269).
 * Per ADR-011 the extension owns its own copy of the model + data and does NOT import from
 * `best-ball-manager/`. The one functional difference: in the extension a player's `team` is
 * already an NFL abbreviation ("MIN"), so `getByeWeek` treats `team` as an abbreviation
 * directly — there is no `teamToAbbr` dependency.
 *
 * The Eliminator is a weekly head-to-head survival contest (6-of-12 Week-1 pod → 1v1 weeks),
 * which inverts season-long best-ball logic: floor over ceiling, a fixed 3 QB / 5 RB / 6–7 WR /
 * 3–4 TE shape, late (Week 13/14) byes as a high-leverage lever, a staggered "bye rainbow"
 * (no two same-position players share a bye), and systematic fades of late-developing rookies /
 * contingent backs.
 *
 * Nothing here re-ranks the board — per ADR-010 the candidate list is annotated, not reordered.
 * These functions only describe state and flag format-specific risks.
 */
...
// Resolve a player's bye week from their team ABBREVIATION (e.g. "MIN").
// Returns null when the team is unknown/unresolved — callers omit bye annotations in that case.
...
// Per-position progress vs. the Eliminator target shape.
...
// A "collision" = two+ players at the SAME position sharing a bye week (breaks the rainbow).
// "earlyStacks" = two+ players (any position) stacked on an early bye (wk5–8), the
// compounding-zero trap. lateByeCount = picks on a premium/strong (wk13/14) bye.
// Each pick must carry a `team` abbreviation; picks with an unresolved team are counted in
// `unknownByeCount` and otherwise ignored (graceful degradation — ADR-011).
...
// `candidate` and each pick should carry a `team` abbreviation; bye-based fields are null/false
// when the team is unknown (graceful degradation). Fades and onesie-need are team-independent.
