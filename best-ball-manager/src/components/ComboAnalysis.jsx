...
export default function ComboAnalysis({ rosterData = [] }) {
  ...
  const processedData = useMemo(() => {
    ...
    // -------------------------
    // STARTS LOGIC (Early Picks) — R1+R2 with optional R3 breakdown
    // -------------------------
      ...
      if (r1 && r2) {
        ...
        if (!startsMap.has(key2)) {
          startsMap.set(key2, {
            ...
            players2,
            count: 0,
            r3Map: new Map(), // name -> { playerObj, count }
            r3Total: 0
          });
        }
  ...
  // Helper to toggle expand for a combo key
  ...
  // Tab click handler — reset filters sensibly (mirror existing behavior for qbFilter)
  const onTabClick = (t) => {
    setActiveTab(t);
    setQbFilter('');
    setStartSearch('');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {activeTab === 'stacks' ? (
          ...
          /* STARTS as Card Grid with expandable R3 breakdown + quick search */
          <div style={{ maxHeight: '65vh', overflowY: 'auto', padding: 16 }}>
            {/* compute filtered & sorted topN */}
            {(() => {
              // processedData is an array of starts entries
              ...
              return (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
                  {top.map(entry => {
                    ...
                    const barPct = Math.max(6, (entry.count / maxCount) * 100); // min bar visibility
