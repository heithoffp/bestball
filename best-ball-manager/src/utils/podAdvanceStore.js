...
//   1. the math runs in a Web Worker (main thread never blocks; inline
//      chunked fallback when workers are unavailable),
//   2. results are memoized per data-version in module memory, so tab
//      revisits within a session render instantly, and
//   3. results persist to IndexedDB (modelCache.js), so page reloads render
...
export function computePodAdvance({ boards = [], metaById = {}, adp, actuals = null, versionKey, persist = true }) {
  ...
  const runWorker = () => new Promise((resolve) => {
    ...
    try {
      worker = new Worker(new URL('../workers/podAdvance.worker.js', import.meta.url), { type: 'module' });
    } catch {
      resolve(false);
      return;
    }
    worker.onmessage = (e) => {
      if (cancelled) return;
      if (e.data?.type === 'batch') merge(versionKey, e.data.results, persist);
      else if (e.data?.type === 'done') resolve(true);
    };
    worker.onerror = () => resolve(false);
    try {
      worker.postMessage({ boards: todo, metaById, adp, actuals });
    } catch {
      resolve(false); // non-cloneable payload — fall back inline
    }
  });

  // Fallback path: same computation, chunked on the main thread with yields.
  ...
  const promise = (async () => {
    ...
    const workerOk = typeof Worker !== 'undefined' ? await runWorker() : false;
    if (!workerOk && !cancelled) await runInline();
  })().finally(() => {
    release();
    worker?.terminate();
  });
  ...
  return {
    ...
    cancel: () => {
      ...
      release();
      worker?.terminate();
      resolveWorker?.(true); // unblock the promise chain
    },
  };
}
